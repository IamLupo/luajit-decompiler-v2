...
Ast::ConditionBuilder::ConditionBuilder(const TYPE& type, Ast& ast, const uint32_t& endTargetLabel, const uint32_t& trueTargetLabel, const uint32_t& falseTargetLabel)
	: ast(ast), type(type)
{
	if (type == ASSIGNMENT)
	{
...
Ast::ConditionBuilder::~ConditionBuilder()
{
...
Ast::ConditionBuilder::Node*& Ast::ConditionBuilder::new_node(const Node::TYPE& type)
{
	return nodes.emplace_back(new Node(type));
}

Ast::ConditionBuilder::Node::TYPE Ast::ConditionBuilder::get_node_type(const Bytecode::BC_OP& instruction, const bool& swapped)
{
	switch (instruction)
	{
...
void Ast::ConditionBuilder::add_node(const Node::TYPE& type, const uint32_t& nodeLabel, const uint32_t& targetLabel, std::vector<Ast::Expression*>* const& expressions)
{
...
bool Ast::ConditionBuilder::link_nodes()
{
	switch (type)
	{
	case ASSIGNMENT:
	...
	{
		if (conditionNodes[i]->targetLabel == Ast::INVALID_ID)
		{
			continue;
		}
...
void Ast::ConditionBuilder::fix_return_nodes()
{
	switch (type)
	{
	case ASSIGNMENT:
		for (uint32_t i = conditionNodes.size() - 1; i--;)
		{
			...
			case Node::FALSE_TARGET:
				if (conditionNodes[i]->type == Node::FALSY_TEST)
				{
	...
	case STATEMENT:
		for (uint32_t i = conditionNodes.size() - 1; i--;)
		{
			if (conditionNodes[i]->targetNode->type == Node::FALSE_TARGET)
			{
				conditionNodes[i]->inverted = true;
			}
		}
...
bool Ast::ConditionBuilder::build_boolean_logic()
{
	for (uint32_t i = conditionNodes.size() - 1; --i;)
	{
		if (conditionNodes[i - 1]->targetNode == conditionNodes[i] && conditionNodes[i]->incomingNodes == 1)
		{
			if (Node::TYPE_PREFERENCE[conditionNodes[i - 1]->type][conditionNodes[i - 1]->inverted] != 3)
			{
				invert_node(conditionNodes[i - 1]);
			}
		...
		else if (!conditionNodes[i]->incomingNodes)
		{
			...
			{
				if (conditionNodes[i - 1]->inverted != conditionNodes[i]->inverted && !invert_any_node(conditionNodes[i - 1], conditionNodes[i]))
				{
					return false;
				}
...
bool Ast::ConditionBuilder::invert_any_node(Node* const& leftNode, Node* const& rightNode)
{
...
void Ast::ConditionBuilder::invert_node(Node* const& node)
{
...
Ast::ConditionBuilder::Node* Ast::ConditionBuilder::copy_node(Node* const& node)
{
...
void Ast::ConditionBuilder::merge_nodes(Node* const& node, Node* const& targetNode)
{
...
Ast::Expression* Ast::ConditionBuilder::build_expression(Node* const& node)
{
	switch (node->type)
	{
	case Node::LESS_THAN:
...
Ast::Expression* Ast::ConditionBuilder::build_not(Ast::Expression* const& operand)
{
	Ast::Expression* const expression = ast.new_expression(AST_EXPRESSION_UNARY_OPERATION);
	expression->unaryOperation->type = AST_UNARY_NOT;
	expression->unaryOperation->operand = operand;
	return expression;
}

Ast::Expression* Ast::ConditionBuilder::build_binary(const Node::TYPE& type, Ast::Expression* const& leftOperand, Ast::Expression* const& rightOperand)
{
	Ast::Expression* const expression = ast.new_expression(AST_EXPRESSION_BINARY_OPERATION);
...
Ast::Expression* Ast::ConditionBuilder::build_condition()
{
	if (link_nodes())
	{
		fix_return_nodes();
